...
def train_basic(split_dir="../data", 
    ...
    print(f"Fold {cv_fold} - Target: {target_columns[0]}...")
...
def main(argv):
    ...
    for i, arg in enumerate(argv):
        ...
        # Logic to toggle basic vs deep learning if needed
        # if arg.replace('–', '-') == '--chemprop':
        #     basic = False
    ...
    if basic:
        ...
        # Placeholder for deep learning branch if you add it back
        for cv in range(cv_num):
            split_dir = '../data/crossval_splits/' + split_folder + '/cv_' + str(cv)
            save_dir = split_dir + '/model_' + str(cv)
            # train_cm(split_dir=split_dir, epochs=epochs, save_dir=save_dir, target_columns=target_cols)
            pass
