...
# ------------------------------------------------------------------------------
# 0. HELPER: Embedding Extraction (CPU Safe)
# ------------------------------------------------------------------------------
def get_embeddings(model, tokenizer, smiles_list, max_length=128):
    """
    Extracts [CLS] embeddings using CPU to avoid MPS bugs.
    """
    model.to("cpu")
    model.eval()
...
# ------------------------------------------------------------------------------
# 1. MAKE PREDICTIONS (ChemBERTa + XGBoost Pipeline)
# ------------------------------------------------------------------------------
def make_preds(
    model_dir="../data",
    data_dir="../data",
    ...
    df_test=None, 
    preds_dir="../data",
    rf=False, # Argument kept for compatibility, but ignored
    target_columns=["quantified_toxicity"]
):
    print(f"Running Hybrid Inference for CV {cv}...")
    ...
    # 2. Load Fine-Tuned ChemBERTa
    ft_model_path = os.path.join(model_dir, f"model_{cv}", "fine_tuned_chemberta")
    if not os.path.exists(ft_model_path):
        raise FileNotFoundError(f"ChemBERTa model not found at {ft_model_path}")

    tokenizer = AutoTokenizer.from_pretrained(ft_model_path)
    model = AutoModelForSequenceClassification.from_pretrained(ft_model_path)

    # 3. Generate Embeddings
    print("  - Generating embeddings...")
    embeddings = get_embeddings(model, tokenizer, smiles_list)

    # 4. Load and Process Extra Features
    scaler_path = os.path.join(model_dir, f"model_{cv}", "extra_features_scaler.pkl")
    
    if os.path.exists(scaler_path) and not df_extra.empty:
        ...
        # Concatenate
        X_hybrid = np.hstack([embeddings, X_extra])
    else:
        print("  - No scaler or extra features found. Using embeddings only.")
        X_hybrid = embeddings

    # 5. Load XGBoost Model
    xgb_path = os.path.join(model_dir, f"model_{cv}", "hybrid_xgb.json")
    ...
    preds = xgb_model.predict(X_hybrid)
...
def make_pred_vs_actual_tvt(
    ...
    for cv in range(ensemble_size):
        ...
        # Load Metadata and True Values for merging
        ...
        if os.path.exists(pred_file):
            ...
            # We pass model_path_base as data_dir here because make_preds expects 
            # the folder containing train/valid csvs when tvt != test, 
            # OR the test folder when tvt == test.
            # However, our make_preds logic handles paths carefully.
            # We pass the root folder where {tvt}.csv lives as data_dir.
            current_predictions = make_preds(
                model_dir=f'../data/crossval_splits/{model_folder}/cv_{cv}', # Where models live
                data_dir=data_dir, # Where csvs live
                tvt=tvt,
                cv=cv,
                df_test=df_test,
                preds_dir=preds_dir,
        ...
        # Ensure we attach the correct prediction column
        if pred_col_name not in current_predictions.columns:
            print(f"Warning: Expected prediction column {pred_col_name} not found in preds file.")
            continue
